...
```text
LAN privada -> DNS local aifabric.<service> -> Traefik (master1)
                                       |
       +-------------------------------+------------------------------+
       |                                                              |
...
- Reemplazar `/etc/hosts` por registros DNS explícitos `aifabric.<service> -> master1`; este formato no admite un wildcard DNS convencional.
